"""
tabel_shap.py  –  Table embeddings -> SHAP Interpretability Analysis
======================================================
• Supports both Vanilla and K-Fold OoF embedding methods
• Outputs numpy SHAP values and two standard plots
"""
...
def tabel_encoder_binary(
    ...
    """Returns (X_train, X_test, y_train, y_test, clf); no CSV saving."""
...
def shap_analysis(
    ...
    """KernelExplainer + save numpy & plots"""
    ...
    # 2. Wrapper for prediction function (positive class probability)
    def model_predict(x):
        return clf.predict_proba(x)[:, 1]

    # 3. Create Explainer and calculate SHAP
    explainer = shap.KernelExplainer(model_predict, background)
    shap_values = explainer.shap_values(X_test_emb, nsamples="auto")
    np.save(f"{out_prefix}_values.npy", shap_values)
    print(f"SHAP values saved to {out_prefix}_values.npy, shape={shap_values.shape}")
    ...
    print(f"SHAP plots saved to {out_prefix}_summary_bar.png / _beeswarm.png")

# --------------------------- Main Entry --------------------------- #
if __name__ == "__main__":
    CSV = "ADNI_Tabel.csv"          # <- Your original table
    START_COL = 20
    CLASS0, CLASS1 = "AD", "CN"     # Can be changed to "SMCI", "PMCI"
